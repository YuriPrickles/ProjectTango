...
func _process(delta: float) -> void:
	...
	if Main.main.get_current_room() == room_assigned:
		if not ripe:
			...
			if poison_delay <= 0:
				plr.hurt(1,self)
				poison_delay = POISON_TICK_VALUE - 0.5 * (float(Main.main.get_peril()) / Main.MAX_PRL)
...
class PoisonRect:
	extends Node2D
	var gasberry:Gasberry
	
	func _init(g:Gasberry) -> void:
		gasberry = g
		
	
	func _draw() -> void:
		if gasberry and gasberry.ripe:
			var lvl:Level = Main.main.get_level()
			var proper_room:Branch = lvl.dungeon_layout.rooms[gasberry.room_assigned]
			draw_rect(Rect2(Vector2i(0,-7) - Vector2i(gasberry.position) + (proper_room.position * 8),proper_room.size * 8),Main.colors[11] * max(0.2,abs(1-(gasberry.poison_delay/gasberry.POISON_VALUE))),true,-1,true)
